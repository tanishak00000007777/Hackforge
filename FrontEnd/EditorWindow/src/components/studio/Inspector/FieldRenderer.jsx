...
import { SECTION_STYLE_ALIASES } from "@/builder/sections/sectionStyle";
...
export default function FieldRenderer({
  ...
  const update = (newValue) => {
    ...
    if (target === "props") {
      ...
      // A section's background/padding can also live in `styles`, where an AI
      // edit would out-rank this prop and make the Inspector look broken.
      // Clearing the aliases keeps it last-writer-wins in both directions.
      const aliases = SECTION_STYLE_ALIASES[field.key];
      if (aliases) {
        const cleared = Object.fromEntries(aliases.map((key) => [key, undefined]));
        changes.styles = cleared;
        changes.responsive = { [device]: cleared };
      }
    } else if (target === "styles") {
