import { mapNodeById } from "./treeHelpers";
...
function mergeNodeData(node, changes) {
  ...
  // Structural edits (merge/split/group) replace the child list outright.
  if (changes.children !== undefined) merged.children = changes.children;

  // Fallback: If changes are flat, apply them to props for backward compatibility
  if (changes.props === undefined && changes.styles === undefined && changes.responsive === undefined && changes.hidden === undefined && changes.locked === undefined && changes.children === undefined) {
...
/**
 * Same merge semantics as updateComponentProps, but resolves the node
 * anywhere in the tree. Callers that only hold an id (every AI tool) use this.
 */
export function updateNodeById(components, id, changes) {
  return mapNodeById(components, id, (node) => mergeNodeData(node, changes));
}
...
export {
  moveNode, insertNode, findNodeById, resolveNodeRef, findNodePath, findParentOf, mapNodeById,
  replaceNodeById, removeNodeById, wrapNode, unwrapNode, groupNodes, changeNodeType,
} from "./treeHelpers";
